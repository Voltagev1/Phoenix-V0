...
XeonBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, Here is my honourable Master🤴`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
...
break
            ...
            case '?':
            case 'allmenu':
                let xeonmenuoh = `╭═══ Phoenix-V0 🕊═══⊷
┃❃╭──────────────
┃❃│ `Prefix` : [ ]
┃❃│ `User` :  ${pushname}
┃❃│ `Time` : ${xeonytimewisher}
┃❃│ `Day` : Look at the watch
┃❃│ `Note` : VOLTAGE⚡LORD is the gOD
┃❃│ `Date` : Calendar is here
┃❃│ `Version` : 1.0
┃❃│ `Plugins` : 250
┃❃│ `Ram` : 64 GB
┃❃│ `Alive` : ${runtime(process.uptime())}
┃❃│ `Repo` : https://github.com/VoltageLord/Phoenix-V0
┃❃│ `FOLLOW ME ON GITHUB` : https://github.com/VoltageLord
┃❃╰───────────────
╰═════════════════⊷
